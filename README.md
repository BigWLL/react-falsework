...
```  
### mock数据  
有时候使用mock数据可以在本地进行方便的开发
/config/project.config.json   参数mock ：true/false 控制
### zoom  
当项目中出现由于分辨率设置不合理时，但是自适应开发又来不及时，可以考虑缩放
/config/project.config.json   参数zoom ：true/false 控制
